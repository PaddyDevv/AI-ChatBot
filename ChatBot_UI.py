...
class Gui(customtkinter.CTk):
	def __init__(self, *args, **kwargs):
		...
		self.resizable(1,1)
		self.geometry(f"{295}x{380}")

		# Apikey Label
		self.apikeyLabel = customtkinter.CTkLabel(self, text="Ai Chatbot By PaddyDevv",text_color='light blue')
		...
		# Generate Button
		self.generateResultsButton = customtkinter.CTkButton(self, text="Ask AI Question", command=lambda: threading.Thread(target=self.MessageSender).start())
		self.generateResultsButton.grid(row=5, column=1, columnspan=1, padx=20, pady=20, sticky="ew")

		# Result Box
		self.displayBox = customtkinter.CTkTextbox(self, width=250, height=120)
		self.displayBox.grid(row=6, column=0, columnspan=2, padx=20, pady=20, sticky="nsew")
        

	def MessageSender(self):
		print("New Message Sent To API At ", datetime.now())
		Message = self.ApiKeyEntry.get()
		Resp = Send_Message(Message)
		print("New API Response At  ", datetime.now())
		self.displayBox.insert('0.0', f'\nYou : {Message}\nAI : {Resp}\n')
		self.displayBox.option_clear()
